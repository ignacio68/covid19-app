...
export function getCovid19CountryData(countryCode: string) {
  const filterCovid19CountryData = covid19CountriesDataList.filter(countryData => {countryData.id === countryCode})
  console.log(`filterCovid19CountrData: ${JSON.stringify(filterCovid19CountryData)}`)
